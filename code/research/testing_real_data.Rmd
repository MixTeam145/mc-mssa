...
```{r}
model_wind_fi <- Whittle(wind$RPT, fixed = c(0, NA))
model_wind_fi
...
```{r}
model_wind_arfi <- Whittle(wind$RPT)
model_wind_arfi
...
```{r}
set.seed(1234)
m1_fi <- mcssa(f1, 60, "ev", model0 = model_wind_fi, conf.level = 0.95)
```
...
```
```{r}
model_NileMin <- Whittle(f2, fixed = c(0, NA))
model_NileMin <- list(
...
```{r}
p <- plot(m2) + scale_y_continuous(trans = scales::pseudo_log_trans(base = 10))
