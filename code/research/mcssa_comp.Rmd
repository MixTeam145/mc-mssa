...
```{r eval=FALSE, include=FALSE}
mcssa_sim <- function(M, Ls, basis, signal = 0, ...) {
  N <- model$N
  p.values <- list()
  ...
  for (idx in seq_along(Ls)) {
    result <- foreach (i = 1:M, .combine = "c", .options.RNG = 1) %dorng% {
      ...
      res <- mcssa(f, Ls[idx], basis, conf.level = NULL, ...)
      res$p.value
    }
    p.values[[idx]] <- result
  }
...
```{r eval=FALSE, include=FALSE}
p_noise_ev <- mcssa_sim(M, Ls, "ev", model = model)
p_noise_ev_svd <- mcssa_sim(M, Ls, "ev", model = model, decomposition.method = "svd")
p_signal1_ev <- mcssa_sim(M, Ls, "ev", signal1,  model = model)
p_signal2_ev <- mcssa_sim(M, Ls, "ev", signal2,  model = model)
p_signal3_ev <- mcssa_sim(M, Ls, "ev", signal3,  model = model, decomposition.method = "svd")
p_signal4_ev <- mcssa_sim(M, Ls, "ev", signal4,  model = model, decomposition.method = "svd")

p_noise_ev_est <- mcssa_sim(M, Ls, "ev", fixed = list(phi = NA, d = 0))

p.values_ev <- data.frame(
  noise = unlist(p_noise_ev),
  noise_est = unlist(p_noise_ev_est),
...
```{r eval=FALSE, include=FALSE}
p_noise_cos <- mcssa_sim(M, Ls, "cos", model = model)
p_signal1_cos <- mcssa_sim(M, Ls, "cos", signal1, model = model)
p_signal2_cos <- mcssa_sim(M, Ls, "cos", signal2, model = model)
p_signal3_cos <- mcssa_sim(M, Ls, "cos", signal3, model = model)
p_signal4_cos <- mcssa_sim(M, Ls, "cos",  signal4, model = model)

p_noise_cos_est <- mcssa_sim(M, Ls, "cos", fixed = list(phi = NA, d = 0))

p.values_cos <- data.frame(
  noise = unlist(p_noise_cos),
  noise_est = unlist(p_noise_cos_est),
...
df_ev <- p.values_ev |>
  ...
  reframe(
    ...
    alphaI_est = ecdf(noise_est)(alphas),
...
pdf("../../tex/master/img/alphaI_ev_est.pdf", width = 8, height = 4)
plot_distr(
  alphaI_est ~ signif.level, df_ev,
  xlab = "significance level", ylab = "type I error"
)
dev.off()

pdf("../../tex/master/img/alphaI_cos_est.pdf", width = 8, height = 4)
plot_distr(
  alphaI_est ~ signif.level, df_cos,
  xlab = "significance level", ylab = "type I error"
)
dev.off()
```
