...
mcssa_sim <- function(M, Ls, basis, model, signal = 0, ...) {
  ...
  for (idx in seq_along(Ls)) {
    result <- foreach (i = 1:M, .combine = "c", .options.RNG = 1) %dorng% {
      ...
      res <- mcssa(f, Ls[idx], basis, model = model, est.freq = FALSE, conf.level = NULL, ...)
      res$p.value
    }
    p.values[[idx]] <- result
  }
...
p.values_ev <- data.frame(
  ...
  signal2 = unlist(p_signal1_ev),
  signal3 = unlist(p_signal1_ev),
  signal4 = unlist(p_signal1_ev),
...
```{r eval=FALSE, include=FALSE}
p_noise_cos1 <- mcssa_sim(M, Ls, "cos", model)
p_signal1_cos1 <- mcssa_sim(M, Ls, "cos", model, signal1)
p_signal2_cos1 <- mcssa_sim(M, Ls, "cos", model, signal2)
p_signal3_cos1 <- mcssa_sim(M, Ls, "cos", model, signal3)
p_signal4_cos1 <- mcssa_sim(M, Ls, "cos", model, signal4)

p.values_cos <- data.frame(
  noise = unlist(p_noise_cos),
  signal1 = unlist(p_signal1_cos),
  signal2 = unlist(p_signal1_cos),
  signal3 = unlist(p_signal1_cos),
  signal4 = unlist(p_signal1_cos),
  L = rep(as.character(Ls), each = M),
  row.names = NULL
)
```
...
plot_pvalues <- function(formula, data, ...) {
  xyplot(
    formula,
    data,
    groups = L,
    type = "l",
    auto.key = list(corner = c(1, 0), x = 0.95, y = 0.05),
...
p_ev <- plot_pvalues(
  ...
  xlab = "significance level", ylab = "type I error", main = "Eigenvectors (Toeplitz)"
)
p_ev_svd <- plot_pvalues(
  alphaI_svd ~ signif.level, df_ev,
  xlab = "significance level", ylab = "type I error", main = "Eigenvectors (SVD)"
)
p_cos <- plot_pvalues(
  alphaI ~ signif.level, df_cos,
  xlab = "significance level", ylab = "type I error", main = "Cosines")
...
```{r echo=FALSE}
auc_ev1 <- df_ev |> group_by(L) |> dplyr::summarise(AUC.ev = AUC(alphaI, beta1))
auc_cos1 <- df_cos |> group_by(L) |> dplyr::summarise(AUC.cos = AUC(alphaI, beta1))
...
```{r echo=FALSE}
auc_ev2 <- df_ev |> group_by(L) |> dplyr::summarise(AUC.ev = AUC(alphaI, beta2))
auc_cos2 <- df_cos |> group_by(L) |> dplyr::summarise(AUC.cos = AUC(alphaI, beta2))
...
```{r echo=FALSE}
auc_ev3 <- df_ev |> group_by(L) |> dplyr::summarise(AUC.ev = AUC(alphaI_svd, beta3))
auc_cos3 <- df_cos |> group_by(L) |> dplyr::summarise(AUC.cos = AUC(alphaI, beta3))
...
```{r echo=FALSE}
auc_ev4 <- df_ev |> group_by(L) |> dplyr::summarise(AUC.ev = AUC(alphaI_svd, beta4))
auc_cos4 <- df_cos |> group_by(L) |> dplyr::summarise(AUC.cos = AUC(alphaI, beta4))
