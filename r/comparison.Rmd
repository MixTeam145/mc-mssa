---
title: "Сравнение с другими методами"
...
p.values_signal_mcssa_omega0225 <- list()
for (idx in seq_along(Ls)) {
  result <- foreach (
    i = 1:M,
    .combine = 'c',
  ...
  p.values_signal_mcssa_omega0225[[idx]] <- result
}
...
```{r eval=FALSE, warning=FALSE}
p.value.signal_wavelet_omega0225 <- c()
set.seed(5)
for (i in 1:M) {
  ts.ar <- one.channel.ts(model, 0)
  ts0 <- ts.ar + signal
  p.value.signal_wavelet_omega0225 <- c(p.value.signal_wavelet_omega0225, test.ar1.wavelet(ts0))
}
```
...
for (i in 1:M) {
  ts.ar <- one.channel.ts(model, 0)
  ts0 <- ts.ar
  p.value.noise_box_white <- c(p.value.noise_box, test.ar1(ts0))
}
```

```{r eval=FALSE, include=FALSE}
p.value.signal_box_omega0225 <- c()
set.seed(5)
for (i in 1:M) {
  ts.ar <- one.channel.ts(model, 0)
  ts0 <- ts.ar + signal
  p.value.signal_box_omega0225 <- c(p.value.signal_box_omega0225, test.ar1(ts0))
}
```
...
alphaI_wavelet <- sapply(alphas, function(a) mean(p.value.noise_wavelet_white < a))
...
alphaI_box <- sapply(alphas, function(a) mean(p.value.noise_box_white < a))
...
for (i in seq_along(Ls))
  lines(alphaI_mcssa[[i]][-1], beta_mcssa_omega0025[[i]][-1], lwd = lwds[i], col = clrs[i])
lines(alphaI_box, beta_box, lwd = 3, col = "darkblue")
lines(alphaI_wavelet, beta_wavelet, lwd = 3, col = "brown")
legend(
  x = "bottomright",
  c("L = 10", "L = 32", "L = 64", "L = 96", "L = 115", "box", "wavelet"),
  col = c(clrs, "darkblue", "brown"),
