...
system.time(L_optim <- optimize.mcssa(f, signif.level = 0.05, alphaI_threshold = 0.5, interval = c(100, 190), M = M, G = G))
```
...
for (idx in seq_along(Ls)) {
  result <- foreach (
    ...
    f <- one.channel.ts(model.est, signal)
    res <-
      MonteCarloSSA(
        f = f,
        L = Ls[idx],
