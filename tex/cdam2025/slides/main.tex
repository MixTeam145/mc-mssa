...
\begin{frame}{Problem Statement}
	...
	\bluetext{Methods}:
	\begin{enumerate}
		\item Monte Carlo SSA (MC-SSA)~[Allen \& Smith, 1996; Golyandina, 2023]~--- tests $H_0:\tS=0$.
		\item Singular spectrum analysis (SSA)~[Broomhead \& King, 1985; Golyandina, Nekrutkin and Zhigljavsky, 2001].\medskip
	\end{enumerate}
...
\begin{frame}{Example: Time series with signal}
	$\tX=\tS+\boldsymbol{\xi}$, where $\boldsymbol{\xi}$ is AR(1) with $\phi=0.7$ and $\sigma^2=1$ (red noise), $N=200$,
	$$
...
\begin{frame}{Notations \& Known Results: Automatic Grouping in SSA}
	...
	\bluetext{Solution}: Automatic Grouping in SSA~[Golyandina and Zhigljavsky, 2013].\medskip
	...
	\[
	T\left(\widetilde \tX_i; \omega_1,\omega_2\right)>T_0~\Longrightarrow~\widetilde \tX_i\text{ corresponds to the signal}.
	\]
