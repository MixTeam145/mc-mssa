...
\begin{frame}{Motivation}
	...
	\begin{itemize}
		\item There is a method for automatic trend and periodicity extraction~[Dudnik, 2025]. However, it only works if they dominate over noise.\bigskip
		\item The main aim of this work is to develop a method for automatic signal extraction that does not necessarily dominate. 
	\end{itemize}
...
\begin{frame}{Example: Applying SSA}
	...
	\end{figure}
	\bluetext{Insider information}: the components corresponding to the signal are $1$, $2$, $5$, $6$ and $13$.
\end{frame}
...
\begin{frame}{Notations \& Known Results: Monte Carlo SSA}
	...
	$$
	where $W_1,\ldots, W_L$ are normalized sine waves with equidistant frequencies $\omega_k=k/(2L)$: $V_k=\{\cos(2\pi\omega_k j)\}_{j=1}^L$, $W_k=V_k / \|V_k\|$.\smallskip
	
	Distribution of $\widehat p_k$ under $H_0$ is generally unknown and is estimated via Monte Carlo by modeling $\bm\xi$ with density $f_{\bm\theta}$ (what is $\bm\theta$ equal to?).\smallskip
	
	\bluetext{Result}: $(1-\alpha)$-confidence intervals for each $\widehat p_k$ under $H_0$.\smallskip
	
	\textcolor{red}{Problem}: intervals are liberal due to uncontrolled FWER. \smallskip
...
\begin{frame}{Conclusions}
	To sum up, we:
	\begin{enumerate}
		\item \bluetext{Developed and implemented} autoMCSSA, which automatically extracts a significant signal, plus a modification of the Whittle approach using part of the spectrum.\medskip
		\item \bluetext{Advantage over autoSSA}: autoMCSSA can extract signals whose SSA components are not necessarily dominant.\medskip
	\end{enumerate}
