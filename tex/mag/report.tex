...
\section{Сравнение Monte Carlo SSA при разных моделях шума}
Пусть $\{X_t\}$ --- модель $\mathrm{AR}(1)$, а $\{Y_t\}$ --- модель $\mathrm{ARFIMA}(0, d, 0)$. Будем считать дисперсию белого шума одинаковой для обоих процессов и равной $\sigma^2$. Дисперсии $\{X_t\}$ и $\{Y_t\}$ соответственно равны
\[
\mathsf{D}X_t = \frac{\sigma^2}{1 - \phi^2}, \quad \mathsf{D}Y_t = \sigma^2\frac{\Gamma(1 - 2d)}{\Gamma(1-d)^2}.
\]
...
Пусть $d=0.4$. Тогда при $\phi\approx0.719$ процессы $\{X_t\}$ и $\{Y_t\}$ имеют одинаковую дисперсию. На рис.~\ref{fig:spectrum} изображены спектральные плотности процессов. На нем видно, что процесс $\{Y_t\}$ имеет меньшее значение плотности для всех значений $\omega$, за исключением близких к нулю.

Предполагается, что если рассмотреть сигнал с частотой $\omega:f_Y(\omega) < f_X(\omega)$, то критерий MC-SSA с суррогатными реализациями модели $\mathrm{ARFIMA}(0, d, 0)$ даст мощность больше, чем тот же критерий с суррогатными реализациями модели $\mathrm{AR}(1)$. Убедимся в этом. Пусть длина ряда $N=100$ и
\[
...
\begin{figure}[h!]
	...
	\label{fig:ar1_vs_fi}
\end{figure}
На рис.~\ref{fig:ar1_vs_fi} изображены график ошибок первого рода и ROC-кривые критериев MC-SSA для разных длин окна $L$. По рис.~\ref{fig:ar1_vs_fi_roc} видно, что, действительно, замена модели $\mathrm{AR}(1)$ на $\mathrm{ARFIMA}(0, d, 0)$ увеличила мощность критерия против данной альтернативы. Также по рис.~\ref{fig:ar1_vs_fi_alphaI} заметно, что такой критерий стал более радикальным, а значит может требовать большего числа суррогатных данных $G$ для поправки критерия.
