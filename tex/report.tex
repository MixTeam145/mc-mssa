...
\]
Будем смотреть на графики ошибок первого рода и ROC-кривые критериев для длин окна $L=10$, $20$, $50$, $80$, $90$. Будем воспринимать ROC-кривую как график мощности критерия, к которому был применен алгоритм~\ref{alg:correction}. Рассмотрим несколько примеров.
...
\begin{figure}[h!]
	...
	\begin{subfigure}[t]{0.5\textwidth}
		...
		\caption{Ошибка первого рода (Basic MSSA)}
	\end{subfigure}
	\bigskip
	\begin{subfigure}[t]{0.5\textwidth}
		\centering
...
\begin{table}[h]
	...
	\begin{tabular}{|c>{\centering\arraybackslash}m{1in}c>{\centering\arraybackslash}m{1in} >{\centering\arraybackslash}m{1in}cc|}\hline
		Метод & левые/правые векторы & $L$ & длина векторов & количество векторов & $\alpha_I(\alpha^*)$ & $\beta(\widetilde\alpha^*)$ \\
		\hline
		MSSA & левые & $50$* & $50$ & $50$ & $0.343$ & $0.728$\\
		\hline
		MSSA & правые & $80$* & $42$ & $42$ & $0.558$ & $0.689$\\
		\hline
		Block & левые & $20$* & $162$ & $20$ & $0.157$ & $0.796$ \\
		\hline
		Block & правые & $80$* & $80$ & $42$ & $0.367$ & $0.717$\\
		\hline
		Sum & левые & $90$ & $90$ & $22$ & $0.535$ & $0.806$ \\
		\hline
		Sum & правые & $80$* & $42$ & $42$ & $0.51$ & $0.748$ \\
		\hline
	\end{tabular}
\end{table}

В таблице~\ref{tab:res_mc-ssa} для каждого метода указана оптимальная длина окна (для которой удалось простроить ROC-кривую, звездочкой помечены $L$, которые могут не являться оптимальными), длина векторов для проекции, их количество, оптимальная длина окна, а также ошибка первого рода и мощность поправленного критерия при уровне значимости $\alpha^*=0.1$ для каждого метода. Видно, что MC-MSSA с проекцией на левые или правые векторы обеих модификаций мощнее, чем с проекцией на векторы базового MSSA.
\end{example}
...
\begin{example}~\label{mc-mssa_example2}
	...
	\begin{table}[h]
		...
		\begin{tabular}{|c>{\centering\arraybackslash}m{1in}c>{\centering\arraybackslash}m{1in} >{\centering\arraybackslash}m{1in}cc|}\hline
			Метод & левые/правые векторы & $L$ & длина векторов & количество векторов & $\alpha_I(\alpha^*)$ & $\beta(\widetilde{\alpha}^*)$ \\
			\hline
			Block & левые & $10$* & $182$ & $10$ & $0.177$ & $0.398$ \\
			\hline
			Block & правые & $90$* & $90$ & $22$ & $0.414$ & $0.386$\\
			\hline
			Sum & левые & $20$* & $20$ & $20$ & $0.176$ & $0.397$ \\
			\hline
			Sum & правые & $90$* & $22$ & $22$ & $0.425$ & $0.412$ \\
			\hline
		\end{tabular}
	
	\end{table}
	На рис.~\ref{fig:sum_block_ev_phi3} изображены графики ошибок первого рода и ROC-кривые методов Sum и Block с проекцией на левые векторы, а на рис.~\ref{fig:sum_block_fa_phi3} "--- на правые векторы. Как видно по рисункам, при уменьшении параметра $\varphi$, во-первых, увеличивается радикальность критериев для любой длины окна, а во-вторых уменьшается разброс мощности после поправки. В таблице~\ref{tab:res_mc-ssa_phi3} представлены результаты для этого примера. По таблице видно, что различие в мощности у методов совсем небольшое, но все же наибольшая мощность наблюдается у метода Sum с проекцией на правые векторы. Стоит учесть, что из-за слишком сильной радикальности критериев, все приведенные в таблице $L$ могут не являтся оптимальными.
\end{example}
...
\begin{example}
	...
	\begin{table}[h]
		...
		\begin{tabular}{|c>{\centering\arraybackslash}m{1in}c>{\centering\arraybackslash}m{1in} >{\centering\arraybackslash}m{1in}cc|}\hline
			Метод & левые/правые векторы & $L$ & длина векторов & количество векторов & $\alpha_I(\alpha^*)$ & $\beta(\widetilde{\alpha}^*)$ \\
			\hline
			Block & левые & $20$* & $162$ & $20$ & $0.157$ & $0.597$ \\
			\hline
			Block & правые & $80$* & $80$ & $42$ & $0.367$ & $0.473$\\
			\hline
			Sum & левые & $90$ & $10$ & $90$ & $0.535$ & $0.625$ \\
			\hline
			Sum & правые & $80$* & $42$ & $42$ & $0.51$ & $0.613$ \\
			\hline
		\end{tabular}
	
	\end{table}
	На рис.~\ref{fig:sum_omega0225},~\ref{fig:block_omega0225} изображены ROC-кривые критериев. Если сравнить получившиеся графики с графиками на рис~\ref{fig:sum_fa_r},~\ref{fig:sum_fa_r},~\ref{fig:block_ev_r} и~\ref{fig:block_fa_r}, то видно, что с ростом частоты сигнала в альтернативе увеличивается различие в мощности критериев в зависимости от параметра $L$. Также для наглядного сравнения в таблице~\ref{tab:res_mc-mssa_omega0225} представлены результаты этого примера для оптимального $L$.
\end{example}
