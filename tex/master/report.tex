...
В реальности параметры шума $\bm\xi$ неизвестны и их нужно оценивать. Тогда перед алгоритмом~\ref{alg:multiple_mc-ssa} необходимо оценить параметры модели $\bm\xi$ по исходному ряду. В работе~\cite{Poteshkin2024} показано, что оценка параметров делает данный критерий консервативным и им все еще можно пользоваться.

Можно убрать неинтересующую нас часть спектра, задав диапазоны нерассматриваемых частот $\mathcal{B}=\bigcup\limits_{j}[\omega_{j_1}, \omega_{j_2}]$. Таким образом, в алгоритме~\ref{alg:multiple_mc-ssa} из рассмотрения уйдут все векторы $W_k$, частота $\omega_k$ которых попадает в $\mathcal{B}$. Оценка параметров по части спектра возможна, модифицировав правдоподобие Уиттла~\eqref{eq:whittle_loglik}. Пусть $\bm\varphi$ и $\sigma^2$~--- параметры шума $\bm\xi$. Положим $J_{\mathcal{B}^\complement}=\{j \in \{1,\ldots, m\}:~\omega_j=j/N \not\in \mathcal{B}\}$, где $m=\lfloor (N-1)/2 \rfloor$. Тогда правдоподобие Уиттла по части спектра выглядит следующим образом:
...
\end{equation}
где $f(\omega; \bm\varphi,\sigma^2)$~--- спектральная плотность шума $\bm\xi$, $I(\omega)$~--- периодограмма~\eqref{eq:pgram} исходного временного ряда.

Приведем модификацию алгоритма~\ref{alg:multiple_mc-ssa} по части спектра в алгоритме~\ref{alg:multiple_mc-ssa_part_spectrum}.
...
\subsection{Сравнение способов задания проекционных векторов}

Сравним численно два способа задания векторов для проекции $W_k$ критерия MC-SSA (алгоритм~\ref{alg:multiple_mc-ssa}):
\begin{enumerate}
	\item Собственные векторы матрицы $\bfX\bfX^\rmT$ или $\bfT$~\eqref{eq:toeplitz}.
...
В данном разделе под MC-SSA будем подразумевать алгоритм~\ref{alg:multiple_mc-ssa_part_spectrum} с косинусами~\eqref{eq:cos_proj_vectors} в качестве векторов для проекции.
...
\begin{remark}
	Таким же образом можно определить SSA с проекцией на строки матрицы $\bfM\in\mathbb{R}^{l\times K}$.
\end{remark}
...
По аналогии с алгоритмом~\ref{alg:auto_trend} можно выделять и периодики, если частота сигнала $\omega$ или ее оценка $\hat\omega$ известна. Приведем в алгоритме~\ref{alg:auto_periodic} алгоритм автоматической идентификации периодики по заданной частоте.
...
Построим для этого примера распределение верхних границ предсказательных интервалов (см. алгоритм~\ref{alg:multiple_mc-ssa}) для оцененной и истинной модели шума $\bm\xi$. В качестве $\bm\xi$ возьмем красный шум с параметрами $\phi=0.7$ и $\sigma^2=1$, длина ряда равна $N=200$.
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		\State Инициализируем оценку периодик $\hat\tP$ нулевым рядом, множество индексов $J_\text{Periodics} \coloneq \varnothing$ и $\mathcal{B} \coloneq [0, \omega_0]$.
...
\end{equation}
Тогда рассмотрим следующий выбор $\delta$ в зависимости от частоты $\omega$ и силы модуляции $C$: находим такое первое $n=n^*$, что $\Sigma^{C, \omega}_{n^*} > \Sigma_t$ и $\Sigma^{C, \omega}_{n^*} - \Sigma^{C, \omega}_{n^*-1} < \varepsilon$, и берем $\delta = \delta_n^* \eqcolon \delta^*$. То есть выбираем такое $\delta$, чтобы захватывать б\'{о}льшую часть спектра и чтобы рост вклада при дальнейшем увеличении $\delta$ был относительно незначительным.
...
Рассмотрим пример. Пусть $N=200$, $C=2$, $\omega=0.253$. На рис.~\ref{fig:Delta_example} изображена нормированная периодограмма такого сигнала, красной пунктирной линией обозначена главная частота. Одним цветом покрашены значения $\Sigma^{C, \omega}_n - \Sigma^{C, \omega}_{n-1}$, то есть при переходе от $n$ к $n+1$ количество частот, участвующих в сумме, увеличивается на $2$. Пусть $\Sigma_t=0.9$, $\varepsilon=0.05$ (рост вклада ниже $5\%$ считается незначительным), тогда $\delta^*=0.013$, границы частотного интервала обозначены на графике черными пунктирными линиями.
...
\begin{algorithm}
	...
	\label{alg:delta_opt}
	\Input Длина ряда $N$, частота $\omega$, верхняя граница величины амплитудной модуляции $C_{\max}$, порог $\Sigma_t$, порог приращения $\varepsilon$.\\
	...
	\begin{algorithmic}[1]
		...
		\State Найти такое первое $n=n^*$, что $\Sigma_{n^*}^{C_{\max},\omega}>\Sigma_t$ и $\Sigma_{n^*}^{C_{\max},\omega}-\Sigma_{n^*-1}^{C_{\max},\omega}<\varepsilon$, $\delta^* \coloneq \delta_{n^*}$.
	\end{algorithmic} 
\end{algorithm}
...
\begin{algorithm}[!ht]
	...
	\begin{algorithmic}[1]
		...
		\State Применить алгоритм~\ref{alg:multiple_mc-ssa_part_spectrum} к ряду $\hat{\tS} - \hat{\tT}$. Если нулевая гипотеза не отвергается при уровне значимости $\alpha$, завершить работу алгоритма с $\hat{\tS}=\hat{\tT}$ и $\hat{r}=\hat{r}_\text{Trend}$.
...
\begin{algorithm}[!ht]
	...
	\begin{algorithmic}[1]
		...
		\begin{algsubstates}
			...
			\State \label{alg:auto_periodic_trend_mc_modified_step} Получить множество $J_{\omega^\star}$ и оценку периодики $\tP_{\omega^\star}$ с помощью алгоритма~\ref{alg:auto_periodic} с рядами $\left\{\tX^{(j)}\right\}_{j\in J}$, $\delta=\delta_{\omega^\star}$ и $d=1$.
