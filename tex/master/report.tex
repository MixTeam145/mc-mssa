...
\subsection{Оценка параметров шума}\label{sect:mcssa_est_model}

До сих пор предполагалось, что параметры шума $\bm\xi$ известны. В реальности параметры шума $\bm\xi$ неизвестны и их нужно оценивать. Тогда перед алгоритмом~\ref{alg:multiple_mc-ssa} необходимо оценить параметры модели $\bm\xi$ по исходному ряду. В работе~\cite{Poteshkin2024} показано, что оценка параметров несильно искажает критерий. Для наглядности на рис.~\ref{fig:alphaI_true_est_ev} и~\ref{fig:alphaI_true_est_cos} изображены ошибки первого рода для примера из раздела~\ref{sect:mcssa_comp}, параметры оценивались с помощью метода Уиттла (см. раздел~\ref{sect:whittle}). Как видно из графиков, оценка параметров сделала уменьшила радикальность критерия <<ev>> и сделала консервативным критерий <<cos>>.
...
\begin{figure}[!htbp]
	...
	\end{subfigure}
	\caption{Ошибка первого рода (собственные векторы матрицы $\bfT$)}
...
\begin{figure}[!htbp]
	...
	\begin{subfigure}{0.45\linewidth}
		...
		\caption{Оцененные параметры шума}
	\end{subfigure}
	\caption{Ошибка первого рода (косинусы с равноотстоящими частотами)}
...
Важной проблемой метода SSA является автоматическая идентификация компонент на этапе группировке.
На данный момент, существуют методы автоматической идентификации, основанные на частотных характеристиках компонент []. В частности, методы основаны на доле значений периодограммы компоненты разложения в выбранном диапазоне частот. Тренд определяется как низкочастотная составляющая ряда, на чем основан частотный метод автоматической идентификации []. Что касается периодических компонент, то в частотном подходе нужно знать периоды, что часто нереализуемо, а также мешает возможная амплитудная модуляция входящих в периодику гармоник, что приводит к растеканию по частотам в периодограмме.

Для решения проблемы неизвестных частот периодик используется свойство SSA разложения, состоящее в том, что экспоненциально-модулированной гармоники кроме случая частоты 0.5 соответствуют две компоненты с той же модуляцией и той же частотой. На этом основан периодограммный метод~\cite{Vautard1992}, который использует это свойство. Однако метод не решает проблемы растекания частот при амплитудной модуляции. Для решения этой проблемы в работе~\cite{Zhornikova2018} был предложен метод регулярности углов.
...
По аналогии с алгоритмом~\ref{alg:auto_trend} можно выделять и периодики. Приведем в алгоритме~\ref{alg:auto_periodic} алгоритм автоматической идентификации периодики по заданной частоте.
...
Идея данного метода идентификации колебательной составляющей была предложена в работе~\cite{Zhornikova2018}, подробно описана в статье~\cite{GolyandinaZhornikova2023} и была предложена модификация в работе~\cite{Dudnik2025}. Метод основывается на свойствах следующих специальных мер:
...
% По замечанию 3 из \cite{Zhornikova2018}: равенство $\tau(P, Q) =0$ означает, что все углы между последовательными точками $(p_k, q_k)^{\mathrm{T}}$ двумерной диаграммы векторов $P$ и $Q$ примерно одинаковые. Если при этом нормы этих векторов монотонно меняются по $k$, то на двумерных диаграммах получатся <<спирали>>.

% В случае, когда число экспоненциально-модулированных гармоник~\eqref{eq:em_harmonic} неизвестно, в работе \cite{Zhornikova2018} было предложено отбирать пары сингулярных векторов с
% помощью порога, т.е. к одной гармонике относить те вектора $U_i$ и $U_{i+1}$, значение $\tau(U_i, U_{i+1})$ для которых меньше заданного порога. 
...
В работе~\cite{autoSSA} показано, что оценка тренда устойчива к завышению ранга сигнала $r$. Однако это неверно для оценки периодик: при применении EOSSA компоненты, соответствующие шуму, становятся похожими на периодики. В работе~\cite{Dudnik2025} был предложен метод SSA-Aid, который использует статистический критерий для проверки гипотезы об отсутствии сигнала. Приведем описание метода в алгоритмах~\ref{alg:auto_periodic_trend_mc} и~\ref{alg:auto_ssa}. Заметим, что в оригинальной работе~\cite{Dudnik2025} используется произвольный статистический критерий проверки гипотезы об отсутствии сигнала, однако для простоты описания алгоритмов положим в качестве критерия метод MC-SSA, который превосходит по мощности другие критерии проверки гипотезы об отсутствии сигнала~\cite{Poteshkin2024}.

\begin{algorithm}[!htbp]
	\caption{Алгоритм автоматической идентификации компонент, использующий критерий с проверкой гипотезы о белом или красном шуме~\cite{Dudnik2025}}\label{alg:auto_periodic_trend_mc}
...
% \begin{remark}
% 	В алгоритмах~\ref{alg:auto_periodic_trend_mc} и~\ref{alg:auto_ssa} используется MC-SSA (алгоритм~\ref{alg:multiple_mc-ssa_part_spectrum}), однако в оригинальной работе, использовался произвольный статистический критерий проверки гипотезы об отсутствии сигнала.
% \end{remark}
...
Целью данной работы является расширение алгоритма SSA-Aid~\cite{Dudnik2025}, который будет учитывать и этот случай. Этот алгоритм назовем SSA-AidMC, так как он существенно использует метод Monte Carlo SSA~\cite{Allen1996}. Заметим, что алгоритм SSA-Aid также использует метод MC-SSA, однако в SSA-Aid метод MC-SSA используется только как хороший критерий проверки гипотезы отсутствие сигнала в остатке, а в методе SSA-AidMC он используется для определения частот, которые соответствуют сигналу.
...
В данном главе, как и в предыдущей, под MC-SSA будем подразумевать алгоритм~\ref{alg:multiple_mc-ssa_part_spectrum} с косинусами~\eqref{eq:cos_proj_vectors} в качестве векторов для проекции.
...
\subsection{Выбор оптимальных параметров autoMCSSA}\label{sect:autoMCSSA_opt_param}
...
\subsubsection{Случай неизвестной амплитудной модуляции}
...
\begin{remark}
	Чтобы окончательно удостовериться, что данный подход работает, он был применен на примере из раздела~\ref{sect:autoMCSSA_example}. В результате при $C_{\max}=5$, $\Sigma_t=0.9$, $\varepsilon=0.05$ весь сигнал был успешно выделен. Далее, если не сказано иначе, будем считать $\Sigma_t=0.9$ и $\varepsilon=0.05$.
\end{remark}
