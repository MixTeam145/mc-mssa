...
\begin{figure}[!htbp]
	...
	\end{subfigure}
	\caption{Ошибка первого рода для разных длин окна (собственные векторы матрицы $\bfT$)}
...
\begin{figure}[!htbp]
	...
	\begin{subfigure}{0.45\linewidth}
		...
		\caption{Оцененные параметры шума}
	\end{subfigure}
	\caption{Ошибка первого рода для разных длин окна (косинусы с равноотстоящими частотами)}
...
\begin{algorithm}[H]
	\caption{Алгоритм автоматической идентификации тренда~\cite{AlexandrovGolyandina2004}}\label{alg:auto_trend}
...
\begin{algorithm}[!htbp]
	...
	\begin{algorithmic}[1]
		...
		\begin{algsubstates}
			...
			\State Применить к остатку $\hat{\tN}$ алгоритм~\ref{alg:multiple_mc-ssa_part_spectrum}. Если гипотеза не отвергается при уровне значимости $\alpha$, завершить работу алгоритма с $\hat{\tS} =\hat{\tT} + \hat{\tP}$ и $\hat{r}=\hat{r}_\text{Trend}+\hat{r}_\text{Periodics}$.
		\end{algsubstates}
	\end{algorithmic}
\end{algorithm}
