...
\newcommand{\aif}{autoIOSSA+FOSSA}
\newcommand{\as}{\textsf{SSA-Aid}}
\newcommand{\af}{autoFOSSA}
\newcommand{\aeo}{autoEOSSA}

% \renewcommand{\thealgorithm}{\arabic{algorithm}} % Формат номера (1, 2, 3...)
...
\begin{algorithm}[!ht]
	\caption{Критерий Multiple MC-SSA~\cite{Golyandina2023}}\label{alg:multiple_mc-ssa}
	...
	\Output скорректированные интервалы прогнозирования
	\begin{algorithmic}[1]
...
\begin{remark}\label{remark:mcssa_part_spectrum}
	Можно убрать неинтересующую нас часть спектра, задав множество диапазонов частот $\mathcal{B}=\left\{[\omega_{j_1}, \omega_{j_2}]:~\omega_{j_1}\leqslant\omega_{j_2}\right\}$. Таким образом, из рассмотрения уйдут все векторы $W_k$, частота $\omega_k$ которых попадает в интервал $[\omega_{j_1}, \omega_{j_2}]$ для какого-то $j$.
\end{remark}
...
\chapter{Методы автоматической идентификации компонент, использующие критерий MC-SSA}

\section{Сравнение способов задания проекционных векторов}

Сравним численно два способа задания векторов для проекции $W_k$ критерия MC-SSA (см. алгоритм~\ref{alg:multiple_mc-ssa}):
\begin{enumerate}
	\item Собственные векторы матрицы $\bfX\bfX^\rmT$ или $\bfT$~\eqref{eq:toeplitz} в зависимости от того, являются ли рассматриваемые ряды стационарными в широком смысле (определение~\ref{def:stationary}).
...
где $\mathsf{S}=(s_1,\ldots,s_N)$~--- сигнал, $\bm{\xi}$~--- шум (стационарный в широком смысле процесс с нулевым средним). Но, поскольку это определение не учитывает поведение спектральной плотности шума, обобщим его.
...
\begin{figure}[h!]
	\centering
	\subfloat[Проекция на собственные векторы $\bfT$]{\label{a}\includegraphics[width=.5\linewidth]{img/mcssa_comp_alphaI_ev.pdf}}\hfill
	\subfloat[Проекция на собственные векторы $\bfX\bfX^\rmT$]{\label{b}\includegraphics[width=.5\linewidth]{img/mcssa_comp_alphaI_ev_svd.pdf}}\par
	\subfloat[Проекция на косинусы]{\label{c}\includegraphics[width=.5\linewidth]{img/mcssa_comp_alphaI_cos.pdf}}
	\caption{Ошибка первого рода}
...
\]
Введем понятие односторонней периодограммы $\Pi$:
...
Для ряда $\tX$ длины $N$ и $0\leqslant\omega_1\leqslant\omega_2\leqslant0.5$ определим меру, следуя~\cite{Alexandrov2009}:
...
Поскольку тренду соответствуют низкие частоты, можно относить к тренду те компоненты разложения, у которых вклад низких частот больше некоторого порога. Приведем алгоритм автоматической идентификации тренда, описанный в~\cite{Alexandrov2009}.

\begin{algorithm}[H]
	\caption{Алгоритм автоматической идентификации тренда~\cite{Alexandrov2009}}\label{alg:auto_trend}
	...
	\Output компоненты, относящиеся к тренду, оценка тренда $\hat\tT$.
	\begin{algorithmic}[1]
...
По аналогии с алгоритмом~\ref{alg:auto_trend} можно выделять и периодики, если частота сигнала $\omega$ или ее оценка $\hat\omega$ известна. Поскольку экспоненциально-моделированная гармоника описывается одной или двумя элементарными восстановленными компонентами, учтем это.

\begin{algorithm}[H]
	\caption{Алгоритм автоматической идентификации периодики}\label{alg:auto_periodic}
	\Input Ряды $\tX^{(j)}$, $j\in J$, частота $\omega\in(0, 0.5]$, радиус $\delta \geqslant 0$, порог $T_0\in[0, 1]$, количество компонент $r$. \\
	...
	\begin{algorithmic}[1]
		...
		\State Из множества $J_\text{signif}=\{j\in J:~T_j > T_0\}$ выбрать $\min(r, |J_\text{signif}|)$ элементов, соответствующих рядам с наибольшим $\mathsf{MSS}\left(\tX^{(j)}\right)=\frac{1}{N}\sum^{N}_{i=1}\left(x^{(j)}_i\right)^2$. Обозначим множество этих индексов за $J_\text{Periodic}$.
		\State Вычислить оценку периодики $\hat \tP=\sum\limits_{j\in J_\text{Periodic}} \tX^{(j)}$.
	\end{algorithmic}
\end{algorithm}

\begin{remark}
	В алгоритме~\ref{alg:auto_periodic} ряды $\tX^{(j)}$ не обязательно являются элементарно восстановленными компонентами. Это могут быть уже сгруппированные (например, с помощью метода регулярности углов~\cite{GolyandinaZhornikova2023}) компоненты: в таком случае $r=1$. Если $\tX^{(j)}$~--- элементарные восстановленные компоненты, в качестве параметра $r$ логично взять
	\[
		r(\omega)=
		\begin{cases}
...
Поскольку при оцененных параметрах шума метод MC-SSA может обнаружить не все частоты, принадлежащие сигналу, будем оценивать сигнал последовательно, применяя критерий MC-SSA к остатку до тех пор, пока гипотеза $H_0:\tS=0$ не перестанет отвергаться. Так на каждой итерации алгоритма определяется максимально значимая частота и применяется алгоритм~\ref{alg:auto_periodic}.
...
Используя алгоритм~\ref{alg:auto_periodic}, приведем алгоритм метода автоматической идентификации периодических компонент с помощью критерия MC-SSA.

\begin{algorithm}[H]
	\caption{autoMCSSA}
	\label{alg:autoMCSSA}
	\Input временной ряд $\tX$, элементарные восстановленные компоненты $\tX^{(j)}$, $j\in J$, длина окна $L\in(1, N)$, модель шума $\bm\xi$, порог низких частот $\omega_0$, радиус $\delta \geqslant 0$, порог $T_0\in[0, 1]$.\\
	\Output индексы периодических компонент $J_\text{Periodics}$, оценка периодик $\hat \tP$.
	\begin{algorithmic}[1]
		\State Инициализируем оценку периодик $\hat\tP$ нулевым рядом, множество индексов $J_\text{Periodics} \coloneq \varnothing$ и $\mathcal{B} \coloneq \left\{[0, \omega_0]\right\}$.
		\State Применить алгоритм~\ref{alg:multiple_mc-ssa} к ряду $\tX$ с диапазонами нерассматриваемых частот $\mathcal{B}$ (см. замечание~\ref{remark:mcssa_part_spectrum}). Если нулевая гипотеза не отвергается, завершить работу алгоритма.
		\State Пока $|J|\ne0$:
		\begin{algsubstates}
			\State Вычислить оценку максимально значимой частоты $\omega^\star$ (алгоритм~\ref{alg:signif_freq_est}).
			\State Получить компоненты $J_{\omega^\star}$ и оценку периодики $\tP_{\omega^\star}$ с помощью алгоритма~\ref{alg:auto_periodic} с рядами $\left\{\tX^{(j)}\right\}_{j\in J}$ и $r=r(\omega^\star)$.
			...
			\State Если $|J_{\omega^\star}|=0$, $\mathcal{B} \coloneq \mathcal{B}\cup\left\{\left[\omega^\star - \delta, \omega^\star + \delta\right]\right\}$.
			\State \label{alg:auto_periodic_trend_modified_3} Применить к остатку $\hat{\tN}$ алгоритм~\ref{alg:multiple_mc-ssa} с диапазонами нерассматриваемых частот $\mathcal{B}$. Если гипотеза не отвергается при уровне значимости $\alpha$, завершить работу алгоритма.
...
\begin{figure}[!h]
	\centering
	\includegraphics[width=0.8\textwidth]{img/auto_mcssa_result1.pdf}
...
\subsection{Подходы к выделению сигнала}\label{sect:signal_extraction_approaches}

При описании алгоритма autoMCSSA для выделения сигнала $\tS$ использовалось разложение базового варианта SSA. Можно обобщить этот подход на SSA с проекцией (см. раздел~\ref{sect:ssa_with_proj}), зафиксировав подходящий базис.

В данном разделе будем считать, что сигнал представляет из себя экспоненциально-модулированную гармонику~\eqref{eq:em_harmonic} с $\omega\in(0, 0.5)$. Пусть $\hat\omega$~--- оценка $\omega$. Обозначим
\[
	D_1=\begin{pmatrix}
	...
	\end{pmatrix}\in \mathbb{R}^{L_\text{SSA}}.
\]
...
\section{Выбор оптимальных параметров autoMCSSA}\label{sect:autoMCSSA_opt_param}

Как было показано в разделе~\ref{sect:signal_extraction_approaches}, оптимальная величина частотного интервала зависит от величины $|a|$. Из формулы~\eqref{eq:em_harmonic} видно, что амплитуда сигнала в точке $N$ равна $e^{aN}$, поэтому можно считать, что $C=aN$~--- величина амплитудной модуляции. Пусть $\tilde \Pi(k / N) = \Pi(k / N) / \sum_{k} \Pi(k / N)$~--- нормированная (односторонняя) периодограмма, где $\Pi(k / N)$ определена в формуле~\eqref{eq:pgram_corrected}. Обозначим за $\tilde\Pi^{C, \omega}$ нормированную периодограмму для сигнала с частотой $\omega$ и амплитудной модуляцией $C$. Отсортируем частоты $k/N$ по убыванию расстояния до частоты $\omega$, обозначим их как $\omega_{(m)}.$ Обозначим $\delta_0=|\omega - \omega_{(2)}|$ (расстояние до дальней соседней точки), если $\omega$ не попадает в решетку периодограммы, иначе $\delta_0=0$. Рассмотрим следующие $\delta_n$:
...
\end{equation}
Тогда рассмотрим следующий выбор $\delta$ в зависимости от частоты $\omega$ и силы модуляции $C$: находим такое первое $n$, что $\Sigma^{C, \omega}_n > \Sigma_t$ и $\Sigma^{C, \omega}_n - \Sigma^{C, \omega}_{n-1} < \varepsilon$, и берем $\delta=\delta_n$. То есть выбираем такое $\delta$, чтобы захватывать б\'{о}льшую часть спектра и чтобы рост вклада при дальнейшем увеличении $\delta$ был относительно незначительным. Обозначим выбор такого $\delta$ за $\delta^*$.
...
Рассмотрим пример. Пусть $N=200$, $C=2$, $\omega=0.253$. На рис.~\ref{fig:Delta_example} изображена нормированная периодограмма такого сигнала, красной пунктирной линией обозначена главная частота. Одним цветом покрашены значения $\Sigma^{C, \omega}_n - \Sigma^{C, \omega}_{n-1}$ для $n>1$, то есть при переходе от $n$ к $n+1$ количество частот, участвующих в сумме, увеличивается на $2$. Пусть $\Sigma_t=0.9$, $\varepsilon=0.05$ (рост вклада ниже $5\%$ считается незначительным), тогда $\delta^*(\omega, C, \Sigma_t, \varepsilon)=0.013$, границы частотного интервала обозначены на графике черными пунктирными линиями.

Для проверки, насколько такой выбор $\delta$ оптимален, сравним его с другим (итеративным) подходом к выбору $\delta$: будем увеличивать $n$ до тех пор, пока для $\delta=\delta_{n}$ не обнаружится $r(\omega)$ компонент.

\subsection{Численное сравнение}\label{sect:delta_choice_numerical_research}

Проведем численный эксперимент и посчитаем ошибку восстановления сигнала для обоих способов выбора $\delta$. Не ограничивая общности, будем считать, что $C\geqslant0$, поскольку форма периодограммы для $C$ и $-C$ совпадают. Для исследования были выбраны следующие $C$: $0$ (отсутствие модуляции), $1$ (слабая модуляция), $2$ (умеренная модуляция), $5$ (сильная модуляция). Учитывая, что, помимо $\delta$, существует параметр $T_0$ (см. раздел~\ref{sect:grouping_auto}), выбор которого тоже затруднителен, рассмотрим сетку из возможных значений порога. 

Пусть $N=200$, $\omega=0.25$. Параметры шума $\phi=0.7$, $\sigma^2=4$. Амплитуды $A$ для гармоник выбирать будем так, чтобы их SNR по формуле~\eqref{eq:snr} совпадали и равнялись $0.25$. Для вычисления $\delta^*$ $\Sigma_t=0.9$ и $\varepsilon=0.05$.

\begin{remark}
	Поскольку основное преимущество autoMCSSA~--- умение выделять сигнал с недоминирующими компонентами, частота сигнала и дисперсия белого шума были выбраны так, чтобы сигнал был отделим от шума и чтобы компоненты сигнала в SSA не были доминирующими. Далее в примерах будет рассмотрен сигнал с меньшей частотой, однако стоит учитывать, что в таком случае при корректировке SNR компоненты сигнала выйдут на первые позиции из-за вида спектральной плотности красного шума (невозрастающая функция).
\end{remark}
...
\subsection{Случай неизвестной амплитудной модуляции}
...
Пусть $C_{\max}=5$, $\Sigma_t=0.9$, $\varepsilon=0.05$. На примерах из раздела~\ref{sect:delta_choice_numerical_research} посмотрим на применимость данного подхода. На рис.~\ref{fig:mse_omega025_Cmax} и рис.~\ref{fig:mse_omega02525_Cmax} представлены результаты моделирования. Видно, что оптимальный порог $T_0$ совпадает со случаем истинного $C$. Можно сделать вывод, что выбор $\delta$ по верхней границе для амплитудной модуляции не портит оценку сигнала c $\delta=\delta^*$.

\begin{algorithm}
	\caption{Выбор радиуса частотного интервала при заданной границе величины амплитуднои модуляции}
	\label{alg:delta_opt}
	\Input Длина ряда $N$, частота $\omega$, верхняя граница величины амплитудной модуляции $C_\text{max}$, порог $\Sigma_t$, порог приращения $\varepsilon$.\\
	...
	\begin{algorithmic}[1]
		...
		\State Для $n=0,\ldots, \lfloor N/2 \rfloor$ вычислить $\Sigma_n^{C_\text{max},\omega}$ по формуле~\eqref{eq:Sigma_n}.
		\State Найти такое первое $n$, что $\Sigma_n^{C_\text{max},\omega}>\Sigma_t$ и $\Sigma_{n}^{C_\text{max},\omega}-\Sigma_{n-1}^{C_\text{max},\omega}<\varepsilon$, $\delta^* \coloneq \delta_n$.
	\end{algorithmic} 
\end{algorithm}

\begin{remark}
	Чтобы окончательно удостовериться, что данных подход имеет место быть, он был применен на примере из раздела~\ref{sect:autoMCSSA_example}. В результате при $C_{\max}=5$, $\Sigma_t=0.9$, $\varepsilon=0.05$ весь сигнал был успешно выделен. Далее, если не сказано иначе, будем считать $\Sigma_t=0.9$ и $\varepsilon=0.05$.
\end{remark}

\section{Алгоритм автоматической идентификации компонент}\label{sect:ssa_aid_mc}

Метод autoMCSSA, введенный в разделе~\ref{sect:autoMCSSA}, обладает большим недостатком: если компоненты в разложении SSA неразделимы (это так, например, в случае гармоник с одинаковыми амплитудами), то описанный способ автоматической группировки в разделе~\ref{sect:grouping_auto} перестает работать, поскольку, например, в случае гармоник с одинаковыми амплитудами, спектры элементарных восстановленных компонент смешивается, что не дает гарантий идентификации этих компонент. Можно уменьшить порог $T_0$, но тогда увеличивается риск ложной идентификации шумовых компонент. 

\subsection{Существующий метод}

Для борьбы с неразделимостью компонент предложили использовать вложенное (nestled) разложение матрицы $\bfX$, семейство этих методов~--- Oblique SSA~\cite{SSA_R}. Самым новым и удобным на практике является метод EOSSA (ESPRIT-based Oblique SSA)~\cite{Shlemov2017}. На его основе уже был предложен метод автоматической идентификации компонент: SSA-Aid~\cite{Dudnik2025}. Приведем описание метода в алгоритмах~\ref{alg:auto_periodic_trend},~\ref{alg:auto_periodic_trend_mc} и~\ref{alg:auto_ssa}, на которые будем ссылаться далее.
...
\begin{algorithm}[!ht]
	...
	\begin{algorithmic}[1]
		\State Применить к исходному ряду алгоритм \aeo{}($r, \omega_0, T_{\text{low}}$)~\cite{autoSSA}. Результат: множество индексов $J_\text{Trend}$, которое соответствует тренду. 
		\State Применить к компонентам с индексами из множества $J\setminus J_\text{Trend}$ модифицированный метод регулярности углов~\cite{Dudnik2025} с параметром $t_0$. Результат: множество индексов $J_\text{Periodics}$, которое соответствует периодикам. Множество $J_\text{Periodics}$ состоит из множеств размера $2$ (случай частоты $\omega \neq 0.5$) и множеств размера $1$ (случай частоты $\omega = 0.5$).
	\end{algorithmic}
\end{algorithm}

\begin{algorithm}[!ht]
	\caption{Алгоритм автоматической идентификации компонент, использующий критерий MC-SSA~\cite{Dudnik2025}}\label{alg:auto_periodic_trend_mc}
	...
	\Output разложение оценки сигнала в сумму тренда и периодик $\hat{\tS}=\hat{\tT} + \sum_{k=1}^p\hat{\tP}^{(k)}$, уточненная оценка ранга сигнала $\hat{r}$.
	\begin{algorithmic}[1]
		\State Применить алгоритм~\ref{alg:auto_periodic_trend}. Результат: множества $J_{\text{Trend}}$ и $J_{\text{Periodics}}$, $\hat \tT\coloneq \sum\limits_{j\in J_\text{Trend}} \tX^{(j)}$.
		\State Инициализировать оценку периодик $\hat{\tP}$ нулевым рядом, оценку ранга периодической компоненты $\hat{r}_\text{Periodics} \coloneq 0$ и $\mathcal{B} \coloneq \{[0, \omega_0]\}$.
		\State Применить алгоритм~\ref{alg:multiple_mc-ssa} к ряду $\hat{\tS} - \hat{\tT}$ с диапазонами нерассматриваемых частот $\mathcal{B}$ (см. замечание~\ref{remark:mcssa_part_spectrum}). Если нулевая гипотеза не отвергается, завершить работу алгоритма с $\hat{\tS}=\hat{\tT}$ и $\hat{r}=\hat{r}_\text{Trend}$.
		...
		\begin{algsubstates}
			\State $\hat{\tP} \coloneq \hat{\tP} + \tX^{(j)}$. $\hat{r}_\text{Periodics} \coloneq \hat{r}_\text{Periodic}+r_p^{(j)}$, $\hat{\tN} \coloneq \tX - \hat{\tT} - \hat{\tP}$.
			\State Применить к остатку $\hat{\tN}$ алгоритм~\ref{alg:multiple_mc-ssa} с диапазоном нерассматриваемых частот $\mathcal{B}$. Если гипотеза не отвергается при уровне значимости $\alpha$, завершить работу алгоритма с $\hat{\tS} =\hat{\tT} + \hat{\tP}$ и $\hat{r}=\hat{r}_\text{Trend}+\hat{r}_\text{Periodics}$.
		\end{algsubstates}
	\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[!ht]
	...
	\Output разложение оценки сигнала в сумму тренда и периодик $\hat{\tS}=\hat{\tT} + \sum_{k=1}^p\hat{\tP}^{(k)}$, оценка ранга сигнала $\hat{r}$.
	\begin{algorithmic}[1]
...
Можно заметить, что в алгоритме~\ref{alg:auto_periodic_trend_mc} в качестве критерия проверки нулевой гипотезы используется MC-SSA. Однако в алгоритме не используется информация о значимых частотах, и поэтому, вообще говоря, метод может идентифицировать шумовые компоненты. Предлагаемый алгоритм~\ref{alg:auto_periodic_trend_mc_modified} представляет собой модификацию алгоритма~\ref{alg:auto_periodic_trend_mc}, которая лишена подобного недостатка и которая отличается только последним шагом.

\begin{algorithm}[!ht]
	\caption{Модификация алгоритма автоматической идентификации компонент, использующего критерий MC-SSA}\label{alg:auto_periodic_trend_mc_modified}
	\Input временной ряд $\tX$, длина окна $L_\text{SSA}$, длина окна $L_\text{MC-SSA}$, модель шума $\bm\xi$, оценка ранга сигнала $r$, порог $t_0$, порог $p_0$, уровень значимости $\alpha$, порог низких частот $\omega_0$, порог $T_{\text{low}}$, порог $T_0$, верхняя граница величины амплитудной модуляции $C_\text{max}$.\\
	\Output разложение оценки сигнала в сумму тренда и периодик $\hat{\tS}=\hat{\tT} + \sum_{k=1}^p\hat{\tP}^{(k)}$, уточненная оценка ранга сигнала $\hat{r}$.
	\begin{algorithmic}[1]
		\setlineformat{1--4:} 
		\State Шаги аналогичны шагам 1--4 алгоритма~\ref{alg:auto_periodic_trend_mc}.
		\setlineformat{\arabic{ALG@line}:}
		...
		\begin{algsubstates}
			...
			\State \label{alg:auto_periodic_trend_mc_modified_step} Получить множество $J_{\omega^\star}$ и оценку периодики $\tP_{\omega^\star}$ с помощью алгоритма~\ref{alg:auto_periodic} с рядами $\left\{\tX^{(j)}\right\}_{j\in J}$, $\delta=\delta_{\omega^\star}$ и $r=1$.
			\State $\hat{\tP} \coloneq \hat{\tP} + \tP_{\omega^\star}$, $\hat{\tN} \coloneq \tX - \hat{\tP}$.
			\State Если $|J_{\omega^\star}|=0$, $\mathcal{B} \coloneq \mathcal{B}\cup\left\{\left(\omega^\star - \delta_{\omega^\star}, \omega^\star + \delta_{\omega^\star}\right)\right\}$.
			\State Применить к остатку $\hat{\tN}$ алгоритм~\ref{alg:multiple_mc-ssa} с диапазонами нерассматриваемых частот $\mathcal{B}$. Если гипотеза не отвергается при уровне значимости $\alpha$, завершить работу алгоритма с $\hat{\tS} =\hat{\tT} + \hat{\tP}$ и $\hat{r}=\hat{r}_\text{Trend}+\hat{r}_\text{Periodics}$.
...
\begin{remark}
	На шаге~\ref{alg:auto_periodic_trend_mc_modified_step} алгоритма~\ref{alg:auto_periodic_trend_mc_modified} используется $r=1$ компонент, поскольку ряды $\{\tX^{(j)}\}_{j\in J_\text{Periodics}}$ в этом случае представляют собой периодики, а не элементарные восстановленные компоненты.
\end{remark}

\begin{remark}\label{remark:auto_ssa_modified}
	В работе~\cite{Dudnik2025} в алгоритме~\ref{alg:auto_ssa} в качестве $\mathcal{A}$ используется алгоритм~\ref{alg:auto_periodic_trend_mc}. Далее под SSA-Aid будем подразумевать именно такой вариант алгоритма.
\end{remark}
\begin{remark}
	 Можно использовать в алгоритме~\ref{alg:auto_ssa} вместо алгоритма~\ref{alg:auto_periodic_trend_mc} алгоритм~\ref{alg:auto_periodic_trend_mc_modified}. Таким образом, можно добиться меньшего количества запусков алгоритма MC-SSA и меньшей чувствительности метода к выбору порога $t_0$, отвечающего за отбор пар гармоник. 
\end{remark}

Недостаток алгоритма~\ref{alg:auto_ssa}~--- неумение идентифицировать недоминирующие компоненты, например, 13-я компонента на рис.~\ref{fig:reconstructed_ts}. Однако с помощью этих алгоритмов можно идентифицировать первые $r$ компонент, которые соответствуют сигналу, а затем оставшиеся компоненты идентифицировать отдельно, например, с помощью autoMCSSA. Таким образом, приведем алгоритм SSA-AidMC (SSA Automatic Identification using MC-SSA).

\begin{algorithm}[H]
	\caption{SSA-AidMC}\label{alg:ssa_aid_mc}
	\Input временной ряд $\tX$, длина окна $L_\text{SSA}$, длина окна $L_\text{MC-SSA}$, оценка ранга сигнала $r$, порог $t_0$, порог $p_0$, уровень значимости $\alpha$, порог низких частот $\omega_0$, порог $T_{\text{low}}$, порог $T_0$, верхняя граница величины амплитудной модуляции $C_\text{max}$.\\
	\Output разложение оценки сигнала в сумму тренда и периодик $\hat{\tS}=\hat{\tT} + \sum_{k=1}^p\hat{\tP}^{(k)}$, уточненная оценка ранга сигнала $\hat{r}$.
	\begin{algorithmic}[1]
		\State Применить алгоритм~\ref{alg:auto_ssa} с $\mathcal{A}$ равным алгоритму~\ref{alg:auto_periodic_trend_mc_modified}. Результат: оценка сигнала $\hat{\tS}_1=\hat\tT + \hat\tP$, ранг $\hat r_1=\hat{r}_\text{Trend}+\hat{r}_\text{Periodics}$ и элементарные восстановленные компоненты $\left\{\tX^{(j)}\right\}_{j=\hat r_1+1}^{r}$, соответствующие остатку $\tX-\hat{\tS}_1$.
		...
		\State Определить максимальный индекс, соответствующий сигналу: $j_{\max}=\max J_\text{Periodics}$.
		\State Применить алгоритм~\ref{alg:auto_periodic_trend_mc_modified} с $r=j_{\max}$. Результат: оценка сигнала $\hat \tS = \hat\tT + \hat\tP$ и оценка ранга сигнала $\hat{r}=\hat{r}_\text{Trend}+\hat{r}_\text{Periodics}$.
	\end{algorithmic}
\end{algorithm}
...
\tX = \tT + \tP + \bm{\xi},
\]
где $N=200$, $\tT=(t_1,\ldots, t_N)$~--- тренд, $\tP=(p_1,\ldots, p_N)$~--- периодика, $\bm\xi$~--- красный шум с параметрами $\phi$ и $\sigma^2$.
...
Рассмотрим примеры сигналов, доминирующих над шумом, то есть сигналу соответствует первые $r$ компонент, где $r$~--- ранг сигнала. Для таких сигналов выбраны следующие параметры алгоритмов:
\begin{enumerate}
...
\begin{example}\label{eg:signal1}
	...
	\begin{table}[h!]
		...
		\caption{MSE восстановления сигнала, пример~\ref{eg:signal1}}
		\label{tab:mse_signal1}
	\end{table}
\end{example}
...
\begin{example}\label{eg:signal2}
	...
	\begin{table}[h!]
		...
		\caption{MSE восстановления сигнала, пример~\ref{eg:signal2}}
		\label{tab:mse_signal2}
	\end{table}
\end{example}

В таблице~\ref{tab:mse_signal1} и~\ref{tab:mse_signal2} представлены среднее и медианное MSE восстановления сигнала и тренда с периодикой по-отдельности. Из этих таблиц можно увидеть следующее:
\begin{enumerate}
	\item Хуже всего справился autoMCSSA. SSA-AidMC хуже SSA-Aid, но различия в ошибках небольшие;
...
Рассмотрим примеры сигналов, некоторые компоненты которых не доминируют над шумом. Для таких сигналов выбраны следующие параметры алгоритмов:
\begin{enumerate}
	\item Длины окна $L_\text{SSA}=96$ и $L_\text{MC-SSA}=48$;
	\item Порог низких частот $\omega_0=1/24$;
	\item Пороги $T_\text{low}=T_0=0.5$;
	\item Максимально возможная модуляция $C_{\max}=5$;
	\item Оценка ранга сигнала $r=30$;
	\item Поскольку шум будет достаточно сильным, порог для алгоритма~\ref{alg:auto_ssa} $t_0=0.5$, а для алгоритма~\ref{alg:ssa_aid_mc} $t_0=+\infty$ (то есть его нет).
\end{enumerate}

\begin{remark}
	Поскольку алгоритм~\ref{alg:auto_ssa} при проверке гипотез не использует значимую частоту для определения сигнальных компонент, а выделяет компоненты с наибольшей суммой квадратов, в случае недоминирующего сигнала метод показывает плохие результаты, вне зависимости от значения порога $t_0$.
\end{remark}
...
\begin{example}\label{eg:signal3}
	...
	\begin{table}[h!]
		...
		\begin{tabular}{c|c|c}
				...
				\hline
		\end{tabular}
		\caption{MSE восстановления сигнала, пример~\ref{eg:signal3} ($\phi=0.7$, $\sigma^2=1$)}
		\label{tab:mse_signal3}
	\end{table}
\end{example}
...
\begin{example}\label{eg:signal4}
	...
	\begin{table}[h!]
		...
		\begin{tabular}{c|c|c}
				...
				\hline
		\end{tabular}
		\caption{MSE восстановления сигнала, пример~\ref{eg:signal4} ($\phi=0.7$, $\sigma^2=1$)}
		\label{tab:mse_signal4}
	\end{table}
\end{example}

Будем рассматривать красный шум с параметрами $\phi=0.7$ и $\sigma^2=1$. В таблицах~\ref{tab:mse_signal3} и~\ref{tab:mse_signal4} представлены среднее и медианное MSE восстановления сигнала. Как видно из таблиц, SSA-AidMC является безоговорочным лидером, имеющим наименьшие ошибки и являющимся более устойчивым (поскольку среднее не сильно больше медианы). autoMCSSA, как и для примеров~\ref{eg:signal1} и~\ref{eg:signal2}, имеет среднее MSE заметно больше медианного. SSA-Aid же оказался неприменимым для данных примеров: ошибки в $8$ раз больше, чем для SSA-AidMC. 

% В таблице~\ref{} представлены результаты методов автоматической идентификации компонент для примеров~\ref{eg:signal3} и~\ref{eg:signal4}.
