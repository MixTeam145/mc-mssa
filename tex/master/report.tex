...
Для реализации рассматриваемых методов использовался язык программирования \textsf{R}~\cite{R} и пакет \textsf{Rssa}~\cite{Rssa}.
...
\begin{definition}
	...
	\end{equation}
	соотношением сигнал-шум, где $I(\omega)$~--- периодограмма $\tS$, $f_{\bm\xi}(\omega)$~--- спектральная плотность $\bm\xi$.
\end{definition}
...
\begin{equation}\label{eq:freq_measure}
	T(\mathsf{X};\omega_1,\omega_2)=\frac{1}{\|\mathsf{X}\|^2}\sum_{k:\omega_1\leqslant k/N\leqslant \omega_2}I(k/N),
\end{equation}
где $I(\omega)$~--- периодограмма ряда $\mathsf{X}$. Величину $T(\mathsf{X},\omega_1,\omega_2)$ можно рассматривать как долю вклада частот, содержащегося в интервале $[\omega_1,\omega_2]$.
