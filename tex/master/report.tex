...
% Тогда рассмотрим следующий выбор $\delta$ в зависимости от частоты $\omega$ и силы модуляции $C$: находим такое первое $n=n^*$, что $\Sigma^{C, \omega}_{n^*} > \Sigma_t$ и $\delta_{n^*}>0$ и берем $\delta = \delta_{n^*} \eqcolon \delta^*$. То есть выбираем такое $\delta$, чтобы захватывать б\'{о}льшую часть спектра и хотя бы $2$ частоты (условие $\delta_{n^*}>0$).

\begin{figure}[h!]
	\centering
	\includegraphics[width=\linewidth]{img/Delta_example.pdf}
	\caption{$\Pi^{C, \omega}(k / N)$ и границы частотного интервала при $\delta=\delta^*$ ($N=200$, $C=2$, $\omega=0.253$, $\Sigma_t=0.9$)}
...
Длину окна $L_\text{MC-SSA}$ для метода MC-SSA следует выбирать так, чтобы мощность критерия была наибольшей. Как было показано в разделе~\ref{sect:mcssa_comp}, если предполагается, что в ряде присутствуют периодики с амплитудной модуляцией, она не должна быть слишком большой, поскольку чем больше $L_\text{MC-SSA}$, тем сильнее растекается частота, соответствующая сигналу и тем меньше мощность критерия. С другой стороны, длина окна должна давать достаточную дискретизацию спектра (напомним, что длине окна $L$ соответствуют частоты $k/(2L)$), чтобы адекватно идентифицировать значимую частоту. Как и в случае $L_\text{SSA}$, если известен предполагаемый период сигнала, стоит выбирать длину окна, кратную этому периоду.   

%  стоит выбирать, кратную периоду предполагаемой гармоники, и не слишком большую, если предполагается, что у гармоники есть амплитудная модуляция.
