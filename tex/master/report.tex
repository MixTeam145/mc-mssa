...
\section{Выбор оптимальных параметров autoMCSSA}

Как было показано в разделе~\ref{sect:signal_extraction_approaches}, оптимальная величина частотного интервала зависит от величины $|\alpha|$. Обозначим за $C=\alpha N$ величину амплитудной модуляции. Величина $C$ имеет следующий смысл: значение гармоники в конце временного промежутка увеличилось в $e^C$ раз.

Пусть $\Pi(k / N) = I^\prime (k / N) / \sum_{k} I^\prime(k / N)$~--- нормированная периодограмма с $I^\prime(k / N)$ из формулы~\eqref{eq:pgram_corrected}. Обозначим за $\Pi^{C, \omega}$ нормированную периодограмму для сигнала с частотой $\omega$ и амплитудной модуляцией $C$. Отсортируем частоты $k/N$ по убыванию расстояния до частоты $\omega$, обозначим их за $\omega_{(1)}, \omega_{(2)}, \ldots.$ Рассмотрим следующие $\delta$:
\begin{enumerate}
	\item Если $\omega < \omega_{(1)}$, то
	\[
	\Delta=\{\omega - \omega_{(k)}~|~\omega_{(k)}\leqslant \omega\}
	\]
	\item В противном случае
	\[
	\Delta=\{\omega_{(k)} - \omega~|~\omega_{(k)}\geqslant \omega\}.
	\]
\end{enumerate}
Упорядочим $\delta\in \Delta$ по возрастанию и обозначим их как $\delta_{(n)}$. Введем частичные суммы вокруг главной частоты:
\[
\Sigma^{C, \omega}_n=\sum_{k:~|\omega_{(k)} - \omega| \leqslant \delta_{(n)}} \Pi^{C, \omega}\left(\omega_{(k)}\right).
\]
Тогда рассмотрим следующий выбор $\delta$ в зависимости от величины $C$: берем такое $\delta^*=\delta_{(n)}$, чтобы $\Sigma^{C, \omega}_n > 0.9$ и $\Sigma^{C, \omega}_n - \Sigma^{C, \omega}_{n-1} < \varepsilon$. То есть выбираем такое $\delta$, чтобы захватывать б\'{о}льшую часть спектра и чтобы рост вклада при дальнейшем увеличении $\delta$ был относительно незначительным.

\begin{figure}[h!]
	\centering
	\includegraphics[width=\linewidth]{img/Delta_example.pdf}
	\caption{$\Pi^{C, \omega}(k / N)$ и <<оптимальные>> границы частотного интервала при $N=200$, $C=2$, $\omega=0.253$, $\varepsilon=0.05$}
	\label{fig:Delta_example}
\end{figure}

Рассмотрим пример. Пусть $N=200$, $C=2$, $\omega=0.253$. На рис.~\ref{fig:Delta_example} изображена нормированная периодограмма такого сигнала, красной пунктирной линией обозначена главная частота. Одним цветом покрашены значения $\Sigma^{C, \omega}_n - \Sigma^{C, \omega}_{n-1}$ для $n>1$ и $\Sigma^{C, \omega}_n$ для $n=1$, то есть при переходе от $n$ к $n+1$ количество частот, участвующих в сумме, увеличивается на $2$. Пусть $\varepsilon=0.05$ (рост вклада ниже $5\%$ считается незначительным), тогда $\delta^*(\omega, C)=0.013$, границы частотного интервала обозначены на графике черными пунктирными линиями.
...
Проведем численный эксперимент и посчитаем ошибку восстановления сигнала для обоих способов выбора $\delta$. Не ограничивая общности, будем считать, что $C\geqslant0$, поскольку форма периодограммы для $C$ и $-C$ совпадают. Для исследования были выбраны следующие $C$: $0$ (отсутствие модуляции), $1$ (слабая модуляция), $2$ (умеренная модуляция), $5$ (сильная модуляция). Учитывая, что, помимо $\delta$, существует параметр $T_0$, выбор которого тоже затруднителен, рассмотрим сетку из возможных значений порога. 

Пусть $N=200$, $\omega=0.25$. Параметры шума $\phi=0.7$, $\sigma^2=4$. Амплитуды $A$ для гармоник выбирать будем так, чтобы их SNR по формуле~\eqref{eq:snr} совпадали и равнялись $0.25$. Для вычисления $\delta^*$ $\varepsilon=0.05$.

\begin{remark}
	Частота сигнала и дисперсия белого шума были выбраны так, чтобы сигнал был отделим от шума и чтобы компоненты сигнала в SSA не были доминирующими. При увеличении частоты сигнала и корректировке SNR компоненты сигнала выйдут на первые позиции из-за вида спектральной плотности красного шума (невозрастающая функция).
\end{remark}
...
Теперь пусть частота не попадает в решетку периодограммы: $\omega=0.2525$ (наихудший случай, частота попала ровно посередине между частотами). Результаты моделирования представлены на рис.~\ref{fig:mse_omega02525}. Как видно, по данным графикам можно сделать аналогичные случаю $\omega=0.25$ выводы.
...
Пусть $C_{\max}=5$. На примерах из раздела~\ref{sect:delta_choice_numerical_research} посмотрим на применимость данного подхода. На рис.~\ref{fig:mse_omega025_Cmax} и рис.~\ref{fig:mse_omega02525_Cmax} представлены результаты моделирования. Видно, что оптимальный порог $T_0$ совпадает со случаем истинного $C$. Можно сделать вывод, что выбор $\delta$ по верхней границе для амплитудной модуляции не портит оценку сигнала c $\delta=\delta^*$.
