...
\begin{remark}
	Чтобы окончательно удостовериться, что данных подход имеет место быть, он был применен на примере из раздела~\ref{sect:autoMCSSA_example}. В результате при $C_{\max}=5$ весь сигнал был успешно выделен. 
\end{remark}
